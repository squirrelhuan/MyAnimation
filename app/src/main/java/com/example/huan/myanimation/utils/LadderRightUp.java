...
public class LadderRightUp extends Animator {
    ...
    public LadderRightUp(int partCount) {
        this.partCount = partCount;
    }
    ...
    public void reDraw(Canvas canvas) {
        ...
        int y = (int) (-height*0.5)+(int)(height*1.5*(mFactor));
        path.moveTo((float) (width*1.5),(float)(-height*0.5));
        path.moveTo(x-(partCount)*cell_width,(float)(-height*0.5));
        ...
        path.lineTo((float) (width*1.5),(float)(-height*0.5));
        //先将canvas保存
        canvas.save();
        //设置为在圆形区域内绘制
