...
public class DiamondClose extends Animator {
    ...
    public DiamondClose(int partCount) {
        this.partCount = partCount;
    }
    ...
    public void reDraw(Canvas canvas) {
        ...
        path.moveTo(width/2,(mFactor)*height/2-height/2*(1-mFactor));
        path.lineTo(width -width*(mFactor)/2+width/2*(1-mFactor),height/2);
        path.lineTo(width/2,height-height*(mFactor)/2+height/2*(1-mFactor));
        path.lineTo((mFactor)*width/2-width/2*(1-mFactor),height/2);
        path.lineTo(width/2,(mFactor)*height/2-height/2*(1-mFactor));
        //先将canvas保存
        canvas.save();
        //设置为在圆形区域内绘制
