...
public class HorizontalChessboard extends Animator {
    ...
    public HorizontalChessboard(int partCount) {
        this.partCount = partCount;
    }
    ...
    public void reDraw(Canvas canvas) {
        ...
        for (int i = 0; i < partCount; i++) {
            for (int j = 0; j < partCount; j++) {
                if (i % 2 == 0) {
                    canvas.drawRect(cell_width * (j+mFactor), cell_height * i, cell_width * (j+1f), cell_height * (i+1), new Paint());
                } else {
                    canvas.drawRect(cell_width * (j+mFactor*1.5f), cell_height * i, cell_width * (j+1.5f), cell_height * (i+1), new Paint());
                }
            }
        }
    }
