...
public class CrossOpen extends Animator {
    ...
    public CrossOpen(int partCount) {
        this.partCount = partCount;
    }
    ...
    public void reDraw(Canvas canvas) {
        ...
        path.addRect(0,0,width/2*mFactor,height/2*mFactor,Path.Direction.CW);//左上
        path.addRect(width-width/2*(mFactor),0,width,height/2*mFactor,Path.Direction.CW);//右上
        path.addRect(width-width/2*(mFactor),height-height/2*(mFactor),width,height,Path.Direction.CW);//右下
        path.addRect(0,height-height/2*mFactor,width/2*mFactor,height,Path.Direction.CW);//左下
        //先将canvas保存
        canvas.save();
        //设置为在圆形区域内绘制
